...
describe("GitHub app navigation", () => {
  ...
  it("keeps removed pull-request files out of live workspace navigation", async () => {
    ...
    // Both paths render as plain text: bb 0.39.0's runtime has no
    // workspace file-open API, so `components/sdk-compat.tsx` renders
    // `FileLink` as a span (see the note there).
    expect(slot.queryByRole("link", { name: "removed.ts" })).toBeNull();
    expect(slot.queryByRole("link", { name: "modified.ts" })).toBeNull();
    expect(slot.getByText("modified.ts")).toBeDefined();
