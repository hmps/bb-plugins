// bb-plugin-github-plus — GitHub issues & pull requests inside BB.
//
...
// The composer's resolved `NewThreadRequest` (@get-bb/plugin-sdk/app).
// Validated loosely on purpose: the host owns this shape and adds fields over
// time, so only the ids the plugin depends on are pinned and everything else
...
export const githubRpcContract = defineRpcContract({
  ...
  /** { repo, number } → the review prompt + project the dialog should seed. */
  reviewPrompt: {
    input: itemInputSchema,
    output: z
      .object({
  ...
  /**
   * { repo, number, request } → spawn a review thread from the composer's
   * resolved NewThreadRequest. Every execution field is forwarded verbatim;
   * `executionInputSources` in particular, because the host drops a requested
   * providerId/model that carries no provenance source.
   */
  spawnReview: {
    input: z
      .object({
...
/** The thread title BB shows for an issue/PR agent thread. */
export function buildThreadTitle(
  repo: string,
  number: number,
  title: string,
): string {
  return `${repo}#${number}: ${title}`.slice(0, 120);
}

/**
 * The review prompt for a pull request. Pure, so both `spawnOnItem` and the
 * `reviewPrompt` rpc that seeds the review dialog build the same text.
 */
...
export function buildIssuePrompt(
  ...
  return [
    `Work on GitHub issue ${repo}#${number}: ${title}`,
    "",
...
export default async function plugin(bb: BbPluginApi) {
  ...
  /** The prompt + resolved project a review dialog needs to seed itself. */
  async function reviewSeed(
    repo: string,
    number: number,
  ): Promise<{ prompt: string; projectId: string; title: string }> {
    const item = getCachedItem("pr", repo, number);
    const title = item?.title ?? `PR #${number}`;
    const projectId = await resolveProjectId(repo);
    return { prompt: buildReviewPrompt(repo, number, title), projectId, title };
  }
  ...
  async function spawnOnItem(
    ...
    const prompt =
      kind === "issue"
        ? buildIssuePrompt(repo, number, title, item?.body ?? null)
        : buildReviewPrompt(repo, number, title);
    const thread = (await bb.sdk.threads.spawn({
      projectId,
      environment: { type: "project-default" },
      title: buildThreadTitle(repo, number, title),
      prompt,
    })) as unknown as SpawnedThreadSummary;
    return await recordSpawn(kind, repo, number, thread.id);
  }

  /**
   * Spawn a review thread from the composer's resolved `NewThreadRequest`.
   * The request is forwarded verbatim — `providerId`, `model`,
   * `reasoningLevel`, `permissionMode`, `serviceTier`, `environment`,
   * `projectId`, `input`, and above all `executionInputSources`, without which
   * the host drops the user's provider/model choice and re-derives it from the
   * project defaults.
   */
  async function spawnReviewFromRequest(
    repo: string,
    number: number,
    request: NewThreadRequestInput,
  ): Promise<{ threadId: string }> {
    const item = getCachedItem("pr", repo, number);
    const title = item?.title ?? `PR #${number}`;
    const thread = (await bb.sdk.threads.spawn({
      ...request,
      title: buildThreadTitle(repo, number, title),
    } as unknown as Parameters<typeof bb.sdk.threads.spawn>[0])) as unknown as SpawnedThreadSummary;
    return await recordSpawn("pr", repo, number, thread.id);
  }

  /** Link the thread to the issue/PR and log it — shared by both spawn paths. */
  async function recordSpawn(
    ...
    bb.log.info(`spawned thread ${threadId} for ${kind} ${repo}#${number}`);
    return { threadId };
  }
  ...
  bb.rpc.register(githubRpcContract, {
    ...
    /** { repo, number } → spawn a review thread on a PR (legacy one-click). */
    async startReview({ repo, number }) {
      return await spawnOnItem("pr", repo, number);
    },

    /** { repo, number } → the prompt + project that seed the review dialog. */
    async reviewPrompt({ repo, number }) {
      return await reviewSeed(repo, number);
    },

    /** { repo, number, request } → spawn a review from the composer request. */
    async spawnReview({ repo, number, request }) {
      return await spawnReviewFromRequest(repo, number, request);
    },
